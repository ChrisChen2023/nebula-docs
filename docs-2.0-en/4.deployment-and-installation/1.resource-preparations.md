...
## Requirements for compiling NebulaGraph
...
You must have the correct version of the software listed below to compile NebulaGraph. If they are not as required or you are not sure, follow the steps in [Prepare software](#prepare_software_for_compiling_nebula_graph) to get them ready.
...
2. Check if the GCC and cmake on your host are in the right version. See [Software requirements](#software_requirements_for_compiling_nebula_graph) for the required versions.
...
## Requirements and suggestions for installing NebulaGraph in test/production environments

### Hardware requirements

| Item               | Requirement for test environments | Requirement for production environments |
| ------------------ | --------------------------------- | --------------------------------------- |
| CPU architecture   | x86_64                            | x86_64                                  |
| Number of CPU core | 4                                 | 48                                      |
| Memory             | 8 GB                              | 256 GB                                  |
| Disk               | 100 GB, SSD                       | 2 * 1.6 TB, NVMe SSD                    |
...
!!! danger
    ...
    In the prodcution environment, **DO NOT** deploy a single cluster across IDCs (The Enterprise Edtion supports data synchronization between clusters across IDCs).

| Process                                   | Suggested number for test environments | Suggested number for production environments |
| ----------------------------------------- | -------------------------------------- | -------------------------------------------- |
| metad (the metadata service process)      | 1                                      | 3                                            |
| storaged (the storage service process)    | 1 or more                              | 3 or more                                    |
| graphd (the query engine service process) | 1 or more                              | 3 or more                                    |
...
FAQ about the above requirements are as shown bellow.
