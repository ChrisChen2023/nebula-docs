...
1. 查看现有备份信息。
  ...
  - 列出在兼容 S3 协议的云存储服务`nebula-br-test`桶下的`/`中的备份的信息。
  ...
  - 列出在谷歌云存储服务`nebula-br-test`桶下的`/`中的备份的信息。
...
2. 使用以下命令恢复数据。
  ...
  - 基于谷歌云存储服务中的备份恢复数据。  
    ...
    将谷歌云存储服务`nebula-br-test`桶下的`/`路径中的`BACKUP_2022_08_12_07_37_02`备份数据，恢复到 Meta 服务的地址为`192.168.8.xxx:9559`的集群中。
  ...
  如果集群[开启 mTLS 认证加密](../../7.data-security/4.ssl.md)，在执行数据恢复命令时，还需要另外添加`--enable-ssl`、`--ca-path`、`--cert-path`、`--key-path`、`--insecure-skip-verify`参数。例如：
  ...
  - 集群开启 mTLS 后，基于兼容 S3 协议的云存储服务中的备份恢复数据。
    ```bash
...
| `--storage` | string | 是 | 无 | 备份数据存储位置，格式为：`<schema>://<storage_path>`。<br>`schema`：可选值为`local`、`s3`和`gs`。<br>选择`s3`时，需要填写`--s3.access_key`、`--s3.endpoint`、`--s3.region`和`--s3.secret_key`。<br>选择`gs`时，需要填写`--gs.credentials`参数。<br>`<storage_path>`：存储位置的路径。|
...
| `--enable-ssl`| - | 否 | 无 | 连接{{nebula.name}}使用 mTLS 加密双向认证。 |
| `--ca-path` | string | 否 | 无 | CA 根证书的存储路径。默认路径为`/usr/local/certs/ca.crt`。 |
| `--cert-path` | string | 否 | 无 | 公钥证书的存储路径。默认路径为`/usr/local/certs/client.crt`。 |
| `--key-path` | string | 否 | 无 | 私钥证书的存储路径。默认路径为`/usr/local/certs/client.key`。 |
| `--insecure-skip-verify` | bool | 否 | 无 | 指定是否跳过验证服务端的证书链和主机名。默认值为`false`，即验证服务端提供的证书链和主机名。如果设置为`true`，则跳过验证。 |
| `--gs.credentials` | string | 否 | 无 | 代表谷歌服务账号密钥的 JSON 字符串。更多信息，可参考[创建和删除服务账号密钥](https://cloud.google.com/iam/docs/keys-create-delete?hl=zh-cn)。 |
